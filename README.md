...
*   **Unified LLM Interface**: `llm request` acts as a single entry point for LLM interactions, delegating prompts to the appropriate provider-specific tool (e.g., `grove-anthropic`, `grove-gemini`) based on the model name specified in flags or configuration.

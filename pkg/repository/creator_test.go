...
func TestValidateCreateOptions(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "invalid repo name - no grove prefix",
			opts: CreateOptions{
				Name:        "test-repo",
				Alias:       "tr",
				Description: "Test repository",
			...
			expectError: true,
			errorMsg:    "repository name must start with 'grove-' prefix",
		},
		{
...
func TestDryRun(t *testing.T) {
	logger := logrus.New()
	logger.SetOutput(ioutil.Discard) // Suppress logs during tests
	creator := NewCreator(logger)
...
func TestGenerateSkeleton(t *testing.T) {
	...
	err := creator.generateSkeleton(opts)
	if err != nil {

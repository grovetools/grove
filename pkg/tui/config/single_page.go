...
type SinglePageOpts struct {
	...
	// It is ignored by bespoke pages such as Themes and Code.
	EssentialsOnly bool
}

// NewSinglePage builds a config Model hosting exactly ONE page, identified
// by its tab ID ("themes", "appearance", "layout", "keys", "code", "notes",
// "join"). The onboarding
...
func NewSinglePage(pageID string, layered *config.LayeredConfig, yamlHandler *setup.YAMLHandler, tomlHandler *setup.TOMLHandler, keys grovekeymap.ConfigKeyMap, opts SinglePageOpts) (Model, error) {
	...
	case "keys":
		...
		page = cp
	case "code":
		m.codePage = NewCodePage(layered, keys, width, height)
		page = m.codePage
	case "notes":
		m.notesPage = NewNotesPage(layered, keys, width, height)
		page = m.notesPage
	case "join":
		m.joinPage = NewJoinPage(layered, keys, width, height)
		page = m.joinPage
	default:

...
// supplementalRegistry contains hand-maintained entries for tools that the
// registry-generator does not pick up yet (it only scans ecosystem projects
// whose grove config sets managed = true). treemux's grove.toml lives in the
// treemux repo, so it is added here for help/listing/delegation purposes
// until it is marked managed and the registry is regenerated.
var supplementalRegistry = map[string]ToolInfo{
	"treemux": {Alias: "treemux", Description: "Unified TUI for the grove workflow"},
}

func init() {
	for repoName, info := range supplementalRegistry {
		if _, exists := toolRegistry[repoName]; !exists {
			toolRegistry[repoName] = info
		}
	}
}
